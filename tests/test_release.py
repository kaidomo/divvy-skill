...
class ReleaseMetadataTests(unittest.TestCase):
    ...
    def test_workflows_validate_before_publishing(self) -> None:
        ...
        self.assertIn("needs: [authorize-and-test, commit-sign-tag]", publish)
        self.assertIn("Revalidate frozen authorization before secret access", publish)
        self.assertIn("AUTHORIZATION_DIGEST", publish)
        self.assertIn("METADATA_SHA", publish)
        ...
        self.assertIn('test "$(trailer Release-Stop-Owner)" = "$INPUT_STOP_OWNER"', release)
        self.assertNotIn('test "$metadata_sha" = "$CHECKOUT_SHA"', release)
        self.assertIn('git fetch origin "refs/tags/$TAG:refs/tags/$TAG"', publish)
        self.assertIn('verify-tag "$TAG" --target "$METADATA_SHA"', publish)
