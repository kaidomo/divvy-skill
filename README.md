...
**primary를 정하기 전에**: 다라운드·신규결정 작업(설계 결정이 예상되거나 CODEX 왕복이 반복될 것 같은 경우)이면 5줄 brief(user_outcome·경계·비책임·stop_condition·예상규모/상한)를 착수 전에 얼린다. 새 결정 없이·되돌리기 쉽고·경계 하나 안에서 끝나는 단순 작업은 생략한다. 자세한 내용은 `SKILL.md` 워크플로 0번.
...
- **단 그 셋은 "돌았다"까지만 보증한다.** 산출물이 요청에 답했는지는 기계가 판정할 수 없다 —
  ...
  **`DIVVY_ISOLATE_CONFIG=1`**로 완전 격리 재실행한다
  (`--ignore-user-config`를 붙여 훅·플러그인·프로필을 로드하지 않는다. 인증은 그대로). 비슷하게,
  Stop 훅이 종료를 계속 거부해 판정을 마치고도 내보내지 못하고 루프하는 경우도 같은 격리 재실행으로
  대응한다.
