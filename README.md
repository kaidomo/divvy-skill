...
**primary를 정하기 전에**: 다라운드·신규결정 작업(설계 결정이 예상되거나, CODEX 왕복이 반복될 것 같거나, 여러 라운드·여러 커밋에 걸칠 것 같은 경우)이면 5줄 brief(user_outcome·경계·비책임·stop_condition·예상규모/상한)를 착수 전에 얼린다. 새 결정 없이·되돌리기 쉽고·경계 하나 안에서 끝나는 단순 작업은 생략한다. 자세한 내용은 `SKILL.md` 워크플로 0번.
...
- **단 그 셋은 "돌았다"까지만 보증한다.** 산출물이 요청에 답했는지는 기계가 판정할 수 없다 —
  회수한 쪽이 읽어야 한다. Codex의 훅·플러그인 preflight가 런을 중단시키고 "리뷰 불가" 한 줄을
  남기는 경우, rc 0 · 파일 존재 · 공백 아님을 모두 통과해도 이런 회수는 성공이 아니라 재실행이나
  CLAUDE 되돌리기 대상이다. 기본 `headless` 프로필 자체가 실패할 때만
  **`DIVVY_ISOLATE_CONFIG=1`**로 완전 격리 재실행한다
  (`--ignore-user-config`를 붙여 훅·플러그인·프로필을 로드하지 않는다. 인증은 그대로). 부모 Codex
  App의 OMX Stop hook이 `identity-indeterminate`를 반복 주입해 종료를 계속 거부하는 경우는 다른
  문제다 — `DIVVY_ISOLATE_CONFIG=1`이 아니라 `scripts/omx_stop_hotfix.py`로 명시적으로 `status`
  후 `apply`한다(`dispatch.sh`가 자동 적용하지 않는다). 자세한 내용은 `SKILL.md` 실행 규약.
